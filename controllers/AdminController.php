...
class AdminController
{
            ...
            if (empty($errors)) {
                ...
                try {
                    ...
                    if ($success) {
                        ...
                        $_SESSION['success_message'] = 'Thêm vở diễn thành công';
                        header('Location: index.php?route=admin/plays');
                        exit;
                    } else {
                        throw new Exception('Thêm vở diễn thất bại');
                    }
                } catch (Exception $e) {
    ...
    public function editPlay()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($_POST['title'])) {
                $errors['title'] = 'Tên vở diễn là bắt buộc';
            }

            if (empty($_POST['theater_id'])) {
                $errors['theater_id'] = 'Nhà hát là bắt buộc';
            }
            ...
            if (empty($_POST['schedules']) || !is_array($_POST['schedules'])) {
                $errors['schedules'] = 'Phai có ít nhất một lịch diễn';
            } else {
                foreach ($_POST['schedules'] as $index => $schedule) {
                    if (empty($schedule['date'])) {
                        $errors["date_$index"] = 'Ngày diễn là bắt buộc';
                    }
                    if (empty($schedule['start_time'])) {
                        $errors["start_time_$index"] = 'Thời gian bắt đầu là bắt buộc';
                    }
                    if (empty($schedule['end_time'])) {
                        $errors["end_time_$index"] = 'Thời gian kết thúc là bắt buộc';
                    }
                }
            }
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
                ...
                if (!in_array(strtolower($ext), $allowed)) {
                    $errors['image'] = 'Kiểu dữ liệu ảnh không tương thích. Yêu cầu: JPG, JPEG, PNG, GIF';
                } else {
                    $upload_dir = 'public/images/plays/';
                    ...
                    if (move_uploaded_file($_FILES['image']['tmp_name'], $destination)) {
                        ...
                        $errors['image'] = 'Không thể tải lên ảnh';
                    }
                }
            }
                ...
                try {
                    ...
                    if ($success) {
                        ...
                        if (isset($_POST['schedules']) && is_array($_POST['schedules'])) {
                            ...
                            if (!$scheduleSuccess) {
                                throw new Exception('Tải lên lịch diễn thất bại');
                            }
                        }
                        ...
                        $_SESSION['success_message'] = 'Cập nhật vở diễn thành công';
                        ...
                        throw new Exception('Cập nhật vở diễn thất bại');
                    }
                } catch (Exception $e) {
                    // Rollback transaction on error
                    $this->conn->rollback();
    ...
    public function deletePlay()
    {
        ...
        $play_id = $_GET['id'] ?? null;
        if (!$play_id) {
            $_SESSION['error_message'] = 'Không có ID vở diễn nào được chỉ định';
            header('Location: index.php?route=admin/plays');
            exit;
        }
        ...
        $play = $this->playModel->getPlayById($play_id);
        if (!$play) {
            $_SESSION['error_message'] = 'Vở diễn không tồn tại';
            header('Location: index.php?route=admin/plays');
            exit;
        }
        ...
        if ($has_bookings) {
            $_SESSION['error_message'] = 'Không thể xóa vở diễn vì đã có đặt vé';
            header('Location: index.php?route=admin/plays');
            exit;
        }
        ...
        if ($success) {
            ...
            $_SESSION['success_message'] = 'Xóa vở diễn thành công';
        } else {
            $_SESSION['error_message'] = 'Xóa vở diễn thất bại';
        }
